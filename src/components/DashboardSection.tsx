...
export default function DashboardSection({
  ...
  return (
    <div id="dashboard-section" className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden mb-14">
      
      {/* Tab bar header */}
      <div className="bg-gray-50/80 px-4 md:px-6 py-4 border-b border-gray-100 flex flex-wrap gap-2 items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="p-2 bg-pink-100 rounded-xl text-pink-600">
            <User className="w-5 h-5" />
          </div>
          <div>
            <h2 className="text-base font-black font-display text-gray-900 tracking-tight">Hi, {user.name}</h2>
            <p className="text-[10px] text-gray-500 font-medium">Logged in as {user.name}</p>
          </div>
        </div>

        <div className="flex gap-1.5 mt-2 sm:mt-0 p-1 bg-gray-200/50 rounded-2xl text-xs font-bold text-gray-600">
          <button
            onClick={() => setActiveTab('student')}
            className={`px-3 py-1.5 rounded-xl transition-all flex items-center gap-1 ${
              activeTab === 'student' ? 'bg-white text-pink-600 shadow-sm' : 'hover:text-gray-900'
            }`}
          >
            👤 Student Hub
          </button>
          {isAdmin && (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl transition-all flex items-center gap-1 ${
                activeTab === 'admin' ? 'bg-white text-purple-600 shadow-sm' : 'hover:text-gray-900'
              }`}
            >
              🛠️ Team Desk
            </button>
        ...
        <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
                <div className="bg-gradient-to-br from-pink-500 to-pink-600 rounded-3xl p-5 text-white flex items-center justify-between relative overflow-hidden group">
                  <span className="absolute -right-6 -bottom-6 w-24 h-24 bg-white/10 rounded-full group-hover:scale-125 transition-transform duration-500" />
                  <div>
                    <span className="text-[10px] font-bold bg-white/20 px-2.5 py-0.5 rounded-full uppercase text-pink-50">
                      Loyalty Points
                    </span>
                ...
                <div className="bg-purple-50 rounded-3xl p-5 border border-purple-100 flex items-center justify-between">
                  <div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <button 
                        ...
                        className="text-[10px] text-pink-600 font-extrabold hover:underline"
                      >
                        {isSavedAddress ? "Edit" : "Save"}
                      </button>
                    </div>
                    {isSavedAddress ? (
                      <p className="text-xs text-gray-700 font-semibold mt-1 flex items-start gap-1">
                      ...
                      <input
                        ...
                        className="w-full text-xs font-semibold p-1.5 bg-white border rounded-lg focus:outline-none"
                      />
                    )}
                  </div>
              ...
              {/* Order History */}
              <div>
                <h3 className="text-xs font-black text-gray-800 uppercase tracking-widest mb-3.5 flex items-center gap-1">
                  <ShoppingBag className="w-4 h-4 text-pink-600" /> Recent Campus Orders
                </h3>

                {orders.length === 0 ? (
                  <div className="p-8 bg-gray-50/40 rounded-3xl border border-dashed border-gray-200/80 text-center text-gray-500">
                  ...
                  <div className="space-y-3">
                    {orders.map((order) => {
                      ...
                      return (
                        <div key={order.id} className="p-4 rounded-3xl border border-gray-100/85 hover:border-gray-200 bg-white shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                          <div className="flex items-start gap-3">
                            <div className="p-2 bg-pink-50 rounded-2xl text-pink-600">
                              <Package className="w-5 h-5" />
                            </div>
                            <div>
                              <div className="flex items-center gap-2">
                                <span className="font-exrabold text-xs text-gray-800 font-bold">Order #{order.id}</span>
                              ...
                              <div className="mt-2 space-y-1">
                                {order.items.map((it, idx) => (
                                  <p key={idx} className="text-xs font-semibold text-gray-700 flex items-center gap-1.5">
                          ...
                          <div className="flex items-center justify-between md:justify-end gap-4 border-t md:border-t-0 pt-3 md:pt-0 border-gray-100">
                            <div>
                              ...
                              <p className="text-xs font-bold text-pink-600 text-right">+{order.pointsEarned} XP</p>
                            </div>
                            <div>
                              <p className="text-[10px] text-gray-400 text-right">Total Charges</p>
                              <p className="text-sm font-black text-gray-900 text-right">₹{Math.round(order.total)}</p>
                            </div>
                            <button
                              type="button"
                              onClick={() => onRepeatOrder(order.id)}
                              className="px-3.5 py-2 bg-pink-50 hover:bg-pink-100 rounded-xl text-pink-700 font-black text-[11px] transition-colors flex items-center gap-1"
                            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 border-t border-dashed border-gray-100 pt-5">
                <div>
                  <h4 className="text-xs font-black text-gray-800 uppercase tracking-widest mb-3 flex items-center gap-1">
                  ...
                  <div className="space-y-2">
                    {user.savedCelebrations.map((celeb) => (
                      <div key={celeb.id} className="p-3 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-between text-xs">
                        <div className="flex items-center gap-2">
                          <div className="p-1.5 bg-white text-purple-600 border rounded-xl">
                            <Gift className="w-4 h-4" />
                          </div>
                          <div>
                            <p className="font-bold text-gray-800">{celeb.name}</p>
                ...
                {/* Add new celebration */}
                <form onSubmit={handleAddCelebSubmit} className="p-4 bg-purple-50/40 rounded-3xl border border-purple-200/55 flex flex-col justify-between">
                  <div>
                    <h5 className="font-bold text-xs text-purple-950 flex items-center gap-1 mb-1">
                      <Plus className="w-4 h-4" /> Add Event Reminder
                    </h5>
                    <p className="text-[10px] text-purple-900/50 mb-3">Save classmate milestones easily</p>
                    ...
                    <div className="space-y-2 mb-3">
                      <input
                        ...
                        className="w-full px-3 py-2 bg-white text-xs rounded-xl focus:outline-none focus:ring-1 focus:ring-purple-400"
                        required
                      />
                      <div className="grid grid-cols-2 gap-2">
                        <select
                          ...
                          className="w-full px-3 py-2 bg-white text-xs rounded-xl focus:outline-none"
                        >
                  ...
                  <button
                    type="submit"
                    className="w-full py-2 bg-purple-600 hover:bg-purple-700 text-white font-extrabold text-xs rounded-xl shadow-md transition-colors"
                  >
                    Quick Add Reminder
                  </button>
                </form>
          ...
          {activeTab === 'admin' && isAdmin && (
            <motion.div
              ...
              className="space-y-6"
            >
              <div className="p-4 bg-purple-50 text-purple-950 border border-purple-100 rounded-3xl flex items-center gap-2">
              ...
              <div className="flex flex-col md:flex-row gap-6">
                <div className="w-full md:w-64 shrink-0 px-2 md:border-r border-gray-100 flex flex-col gap-1.5 md:pr-4">
                  <h4 className="font-extrabold text-xs text-gray-900 uppercase tracking-widest pl-3 mb-2 pt-2">Admin Panel</h4>
                  
                  <button 
                    onClick={() => setActiveAdminTab('analytics')}
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${activeAdminTab === 'analytics' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${activeAdminTab === 'orders' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${activeAdminTab === 'kiosk' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${activeAdminTab === 'catalog' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
                  ...
                  <button 
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${activeAdminTab === 'campus' ? 'bg-purple-50 text-purple-700' : 'text-gray-600 hover:bg-gray-50'}`}
                  >
              ...
              {activeAdminTab === 'orders' && (() => {
                ...
                return (
                  <div className="space-y-6 animate-fadeIn">
                    {/* Header Controls */}
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-5 rounded-3xl border border-gray-150 shadow-sm">
                      <div>
                        <h4 className="font-black text-sm text-purple-950 uppercase tracking-wider">📦 Start-up Order Central</h4>
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                            adminOrderFilter === 'preparing'
                              ...
                              : 'text-blue-600 hover:text-blue-950 hover:bg-blue-50/50'
                          }`}
                        >
                          Packed ({countPacked})
                        </button>
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                            adminOrderFilter === 'delivery'
                              ...
                              : 'text-pink-600 hover:text-pink-950 hover:bg-pink-50/50'
                          }`}
                        >
                        ...
                        <button
                          ...
                          className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all cursor-pointer ${
                            adminOrderFilter === 'ready'
                              ...
                              : 'text-emerald-600 hover:text-emerald-950 hover:bg-emerald-50/50'
                          }`}
                        >
                    ...
                    {filteredOrdersByFilter.length === 0 ? (
                      <div className="p-12 text-center bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                        ...
                        <h4 className="font-extrabold text-xs text-gray-500 uppercase tracking-widest">No matching orders found</h4>
                        ...
                        {filteredOrdersByFilter.map(or => {
                          ...
                          return (
                            <motion.div
                              ...
                              className={`bg-white rounded-3xl border border-gray-200 p-5 shadow-sm transition-all hover:shadow-md ${leftBorder} flex flex-col gap-5`}
                            >
                              {/* Card Header row */}
                              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-gray-100 pb-3">
                                <div>
                                  <div className="flex items-center gap-2">
                                    <span className="font-mono font-black text-sm text-purple-950">Order #{or.id}</span>
                                    <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full border ${badgeBg}`}>
                                      {statusLabel}
                                    </span>
                                    <span className="text-[9px] font-bold bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full border border-gray-200 capitalize">
                                ...
                                <div className="flex items-center gap-2 bg-gray-50/50 p-1.5 rounded-2xl border border-gray-100">
                                  <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest pl-1.5 mr-1">Update State</span>
                                  <select
                                    value={or.status}
                                    onChange={(e) => onUpdateOrderStatus(or.id, e.target.value)}
                                    className="bg-white border text-[11px] font-black py-1 px-2.5 rounded-xl border-purple-200 text-purple-800 focus:outline-none cursor-pointer"
                                  >
                              ...
                              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 text-xs">
                                ...
                                <div className="space-y-3.5 bg-neutral-50/50 p-4 rounded-2xl border border-neutral-100 max-h-72 overflow-y-auto">
                                  <h5 className="font-black text-[10px] text-gray-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-gray-200/50 pb-1.5">
                                    <User className="w-3.5 h-3.5 text-purple-600" /> Courier & Recipient Details
                                  </h5>

                                  <div className="space-y-2.5">
                                    <div className="flex items-center gap-2.5">
                                      <div className="w-7 h-7 rounded-lg bg-white flex items-center justify-center border text-gray-400">
                                        <User className="w-3.5 h-3.5" />
                                      </div>
                                      <div>
                                        <p className="text-[10px] text-gray-400 font-extrabold uppercase tracking-widest">Student Name</p>
                                        <p className="font-extrabold text-gray-800 leading-tight">{or.customerName || 'Campus Student'}</p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2.5">
                                      ...
                                      <div>
                                        ...
                                        <p className="font-bold text-gray-700 leading-tight select-all">{or.userEmail || 'No Email Provided'}</p>
                                      </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2.5">
                                      ...
                                      <div>
                                        ...
                                        <p className="font-bold text-gray-700 leading-tight select-all">{or.customerPhone || 'Not specified'}</p>
                                      </div>
                                    </div>

                                    {/* HIGHLIGHTED DELIVERY LOCATION BLOCK */}
                                    <div className="p-3 bg-red-50/50 rounded-xl border border-red-100 flex gap-2.5 mt-2">
                                      <div className="w-8 h-8 shrink-0 rounded-lg bg-white flex items-center justify-center border border-red-200 text-[#E23744]">
                                        <MapPin className="w-4 h-4 animate-pulse" />
                                      </div>
                                      <div className="flex-1">
                                        <p className="text-[9px] text-[#E23744] font-black uppercase tracking-widest">Hostel Delivery Address</p>
                                        <p className="font-extrabold text-neutral-800 leading-normal mt-0.5">
                                ...
                                <div className="space-y-3.5">
                                  <h5 className="font-black text-[10px] text-gray-400 uppercase tracking-widest flex items-center gap-1.5 border-b border-gray-100 pb-1.5">
                                    <ShoppingBag className="w-3.5 h-3.5 text-purple-600" /> Ordered Items List ({or.items.length})
                                  </h5>

                                  <div className="space-y-3.5 max-h-72 overflow-y-auto pr-1">
                                    {or.items.map((item, idx) => (
                                      <div key={idx} className="flex gap-3 bg-gray-50/50 p-2.5 rounded-2xl border border-gray-100/50 hover:bg-gray-50 transition-colors">
                                        <img
                                          src={item.image}
                                          alt={item.name}
                                          className="w-14 h-14 object-cover rounded-xl border border-gray-250 shrink-0"
                                          referrerPolicy="no-referrer"
                                        />
                                        <div className="flex-1">
                                          <div className="flex justify-between">
                                            <h6 className="font-extrabold text-gray-900 leading-snug">{item.name}</h6>
                                            <span className="font-mono text-purple-950 font-bold shrink-0 ml-2">₹{item.price} × {item.quantity}</span>
                                          </div>
                                          
                                          {item.customization && (
                                            <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1 bg-white p-2 rounded-xl text-[10px] text-gray-600 border border-gray-100">
                                              ...
                                              {item.customization.messageOnCake && (
                                                <p className="sm:col-span-2 text-pink-600 font-bold bg-pink-50/30 px-1.5 py-0.5 rounded border border-pink-100/35">
                                              ...
                                              {item.customization.pickupTime && (
                                                <p className="sm:col-span-2 text-purple-700 font-semibold bg-purple-50/30 px-1.5 py-0.5 rounded border border-purple-100/35 mt-0.5">
                                              ...
                                              {item.customization.photoUrl && (
                                                <div className="sm:col-span-2 mt-1.5 pt-1.5 border-t border-gray-100">
                                                  ...
                                                  <a
                                                    ...
                                                    className="inline-flex items-center gap-1 text-purple-700 hover:text-purple-900 font-bold hover:underline bg-purple-50 px-2 py-1 rounded-lg"
                                                  >
                              ...
                              <div className="border-t border-gray-100 pt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-xs">
                                <div className="text-[10px] text-gray-500 font-semibold">
                                  <span className="mr-3 text-neutral-800">Payment: <strong>{or.paymentMethod || 'UPI/Card'}</strong></span>
                                  ...
                                  <div className="mt-1 text-xs text-gray-900">
                                    Total Paid Amount: <strong className="font-mono text-pink-600 text-sm">₹{or.total}</strong>
                                  </div>
                                </div>
                                ...
                                <div className="flex gap-2">
                                  {or.status === 'placed' && (
                                    <button
                                      ...
                                      className="py-2 px-3.5 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl text-[10px] uppercase tracking-wider transition-all shadow-md shadow-blue-500/10 cursor-pointer text-center select-none"
                                    >
                                      👨‍🍳 Mark Ready & Packed
                                    </button>
                                  )}
                                  ...
                                  {or.status === 'preparing' && (
                                    <button
                                      ...
                                      className="py-2 px-3.5 bg-pink-600 hover:bg-pink-700 text-white font-black rounded-xl text-[10px] uppercase tracking-wider transition-all shadow-md shadow-pink-500/10 cursor-pointer text-center select-none"
                                    >
                                      🚴 Out for Delivery
                                    </button>
                                  )}
                                  ...
                                  {or.status === 'delivery' && (
                                    <button
                                      ...
                                      className="py-2 px-3.5 bg-emerald-600 hover:bg-emerald-700 text-white font-black rounded-xl text-[10px] uppercase tracking-wider transition-all shadow-md shadow-emerald-500/10 cursor-pointer text-center select-none"
                                    >
                                      ✅ Complete & Delivered
                                    </button>
                                  )}
                                  ...
                                  {(or.status === 'ready' || or.status === 'completed') && (
                                    <div className="flex items-center gap-1.5 text-emerald-600 font-extrabold text-[10px] uppercase tracking-wider bg-emerald-50 px-3 py-2 rounded-xl border border-emerald-200">
              ...
              {activeAdminTab === 'kiosk' && (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div className="col-span-12 lg:col-span-5 space-y-5">
                    {editingKioskId ? (
                      <div className="p-5 bg-purple-50/50 rounded-3xl border border-purple-200/60 shadow-md flex flex-col">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="font-extrabold text-xs text-purple-950 uppercase tracking-widest flex items-center gap-1.5">
                            <Settings className="w-3.5 h-3.5 text-purple-700 animate-spin" /> Edit Kiosk Item
                          </h4>
                          <button
                            type="button"
                            onClick={() => setEditingKioskId(null)}
                            className="text-[10px] text-gray-400 hover:text-gray-600 uppercase font-black tracking-wider"
                          >
                            Cancel
                          </button>
                        </div>
                        <p className="text-[11px] text-purple-900/80 mb-4 leading-normal font-medium">
                        ...
                        <form onSubmit={handleSaveKioskEditSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-gray-500 uppercase px-1">Product Name</label>
                            <input
                              type="text"
                              value={editingKioskName}
                              onChange={(e) => setEditingKioskName(e.target.value)}
                              className="w-full px-3 py-2 bg-white text-xs rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-purple-150 transition-colors font-bold text-gray-800"
                              required
                            />
                          </div>
                          <div className="space-y-1">
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-gray-500 uppercase px-1">Flavor / Base</label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-gray-500 uppercase px-1">Total Stock</label>
                            <input
                              type="number"
                          ...
                          <div className="sm:col-span-2 bg-white rounded-xl border border-dashed border-purple-200 p-3 flex flex-col gap-2">
                            ...
                            {editingKioskImage && (
                              <div className="mt-1">
                                <img src={editingKioskImage} alt="Preview" className="h-14 w-14 object-cover rounded-xl shadow-sm border border-gray-200" />
                              </div>
                            )}
                          </div>
                          <button
                      ...
                      <div className="p-5 bg-white rounded-3xl border border-gray-200 shadow-sm flex flex-col">
                        <h4 className="font-extrabold text-xs text-purple-950 uppercase tracking-widest mb-3 flex items-center gap-1">
                          <Plus className="w-3.5 h-3.5 text-purple-600" /> Add to Kiosk Inventory
                        </h4>
                        <p className="text-[11px] text-gray-500 mb-4 leading-normal font-medium">
                        ...
                        <form onSubmit={handleAddNewKioskProductSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          <input
                            ...
                            className="px-3 py-2 bg-gray-50 text-xs rounded-xl border border-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-100 transition-colors text-gray-800 font-medium"
                            required
                          />
                          <input
                            type="number"
                          ...
                          <div className="sm:col-span-2 bg-gray-50 rounded-xl border border-dashed border-gray-300 p-3 flex flex-col gap-2">
                            <label className="block text-[10px] font-bold text-gray-500 uppercase">Product Image</label>
                            <input
                              type="file"
                              accept="image/*"
                            ...
                            {newKioskImage && (
                              <div className="mt-1">
                                <img src={newKioskImage} alt="Preview" className="h-14 w-14 object-cover rounded-xl shadow-sm border border-gray-200" />
                              </div>
                            )}
                          </div>
                          <button
                  ...
                  <div className="col-span-12 lg:col-span-7 p-5 bg-white border border-gray-200 rounded-3xl shadow-sm">
                    <h4 className="font-extrabold text-xs text-purple-950 uppercase tracking-widest mb-1 flex items-center gap-1.5">
                    ...
                    <div className="space-y-3 max-h-[450px] overflow-y-auto pr-1">
                      {kioskInventory.map(item => (
                        <div key={item.id} className="p-3 bg-gray-50 rounded-2xl border border-gray-100 hover:border-purple-200 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                          <div className="flex items-center gap-2.5">
                            <img src={item.image} className="w-10 h-10 rounded-xl object-cover border border-gray-200 shadow-sm" referrerPolicy="no-referrer" />
                            <div>
                              <span className="font-bold text-gray-900 block">{item.name}</span>
                              <span className="text-[10px] text-purple-600 font-semibold uppercase tracking-wider">{item.flavor}</span>
                              <span className="text-[10px] text-gray-500 block">Base Price: ₹{item.price}</span>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-3 border-t sm:border-t-0 pt-2 sm:pt-0">
                            ...
                            <div className="flex items-center gap-1.5">
                              ...
                              <button
                                ...
                                className="w-6 h-6 bg-white hover:bg-red-50 text-red-600 font-extrabold rounded-lg border border-gray-200 hover:border-red-200 transition-colors text-center text-xs flex items-center justify-center active:scale-95"
                              >
                                -
                              </button>
                              <span className="min-w-[40px] text-center font-black text-xs text-gray-900">
                              ...
                              <button
                                ...
                                className="w-6 h-6 bg-white hover:bg-green-50 text-green-600 font-extrabold rounded-lg border border-gray-200 hover:border-green-200 transition-colors text-center text-xs flex items-center justify-center active:scale-95"
                              >
                                +
                              </button>
                            </div>
                            ...
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                ...
                                className="p-1.5 bg-white text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg border border-gray-250 hover:border-red-200 transition-colors active:scale-95"
                              >
                  ...
                  <div className="col-span-12 lg:col-span-5 space-y-5">
                    {editingCakeId ? (
                      <div className="p-5 bg-purple-50/50 rounded-3xl border border-purple-200/60 shadow-md flex flex-col">
                        ...
                        <form onSubmit={handleSaveCakeEditSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          <div className="sm:col-span-2 space-y-1">
                            <label className="text-[10px] font-bold text-gray-500 uppercase px-1">Cake Title</label>
                            <input
                              type="text"
                          ...
                          <div className="space-y-1">
                            <label className="text-[10px] font-bold text-gray-500 uppercase px-1">Base Price (₹)</label>
                            <input
                              type="number"
                              step="0.01"
                          ...
                          <div className="sm:col-span-2 bg-white rounded-xl border border-dashed border-purple-200 p-3 flex flex-col gap-2">
                            ...
                            {editingCakeImage && (
                              <div className="mt-1">
                                <img src={editingCakeImage} alt="Preview" className="h-14 w-14 object-cover rounded-xl shadow-sm border border-gray-200" />
                              </div>
                            )}
                          </div>
                      ...
                      <div className="p-5 bg-white rounded-3xl border border-gray-200 shadow-sm flex flex-col">
                        ...
                        <p className="text-[11px] text-gray-500 mb-4 leading-normal font-medium font-sans">
                        ...
                        <form onSubmit={handleAddNewCakeSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-3 flex-1">
                          ...
                          <select
                            ...
                            className="px-3 py-2 bg-gray-50 text-xs rounded-xl border border-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-100 transition-colors text-gray-800 font-medium sm:col-span-2"
                          >
                            <option value="Birthday Cakes">Birthday Cakes</option>
                            <option value="Chocolate Cakes">Chocolate Cakes</option>
                            <option value="Red Velvet">Red Velvet</option>
                          ...
                          <div className="sm:col-span-2 bg-gray-50 rounded-xl border border-dashed border-gray-300 p-3 flex flex-col gap-2">
                            ...
                            {newCakeImage && (
                              <div className="mt-1">
                                <img src={newCakeImage} alt="Preview" className="h-14 w-14 object-cover rounded-xl shadow-sm border border-gray-100" />
                              </div>
                            )}
                          </div>
                          <button
                        ...
                        <div key={item.id} className="p-3 bg-gray-50 rounded-2xl border border-gray-100 hover:border-purple-200 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              <span className="font-bold text-gray-900 block truncate max-w-[190px]">{item.name}</span>
              ...
              {activeAdminTab === 'campus' && (
                <div className="grid grid-cols-1 gap-5">
                  ...
                  <form onSubmit={handleAddNewCampusSubmit} className="grid grid-cols-1 gap-3 flex-1">
                    <div className="space-y-1">
                      <label className="pl-1 text-[10px] font-bold text-gray-500 uppercase">Institution Name</label>
                      <input
                        type="text"
                        placeholder="e.g. SRCC North Campus"
                        value={newCampusName}
                        onChange={(e) => setNewCampusName(e.target.value)}
                        className="w-full px-3 py-2 bg-gray-50 text-xs rounded-xl border border-gray-200 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100 transition-colors"
                        required
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="pl-1 text-[10px] font-bold text-gray-500 uppercase">Primary Drop-off Hub</label>
                      <input
                        type="text"
                ...
                <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                    <div className="border-b border-gray-100 p-4">
                      <h4 className="font-extrabold text-xs text-gray-900 uppercase tracking-widest flex items-center justify-between">
                        <span>Active Campus Hubs</span>
                        <span className="text-[10px] text-gray-400 bg-gray-50 px-2.5 py-1 rounded-full">{campuses.length} Hubs</span>
                      </h4>
                    </div>
                    <div className="divide-y divide-gray-100 p-3 space-y-2">
                       {campuses.map(campus => (
                         <div key={campus.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                            <div>
                              <h5 className="font-bold text-xs text-gray-900">{campus.name}</h5>
                              <p className="text-[10px] text-gray-500 flex items-center gap-1 mt-0.5"><MapPin className="w-3 h-3"/> {campus.location}</p>
                            </div>
                            <button
                       ...
                       {campuses.length === 0 && (
                         <div className="p-4 text-center text-xs text-gray-500 font-medium">No campuses active.</div>
                       )}
                    </div>
                </div>
                </div>
