  ...
  return (
    <div className="relative z-50">
      ...
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-pink-50 hover:bg-pink-100 text-pink-700 font-semibold rounded-2xl border border-pink-100 shadow-sm transition-all duration-300 group text-sm"
      >
        ...
        <span className="sm:hidden font-bold text-gray-800 text-xs truncate max-w-[120px]">
          {selectedCampus.name.split(' ')[0]}
        </span>
        <ChevronDown className={`w-4 h-4 text-pink-600 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
        ...
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 mt-3 w-80 bg-white rounded-3xl shadow-2xl border border-pink-50/80 p-5 z-50 focus:outline-none"
            >
              <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100">
                <h3 className="font-bold text-gray-800 flex items-center gap-1.5 text-sm">
                  <MapPin className="w-4 h-4 text-pink-600" /> Choose University
                </h3>
                <span className="text-[11px] font-bold text-pink-600 bg-pink-100 px-2 py-0.5 rounded-full flex items-center gap-0.5 animate-pulse">
                  ...
                  return (
                    ...
                    >
                      <div className={`p-2 rounded-xl transition-all ${
                        isSelected ? 'bg-pink-600 text-white' : 'bg-white text-gray-400 border border-gray-200 group-hover:scale-105'
                      }`}>
                        <GraduationCap className="w-4 h-4" />
                      </div>
                      
                      <div className="flex-1 min-w-0">
                        <p className={`font-bold text-xs truncate ${isSelected ? 'text-pink-900' : 'text-gray-700'}`}>
                          {campus.name}
                        </p>
                      ...
                      {isSelected && (
                        <CheckCircle2 className="w-4 h-4 text-pink-600 self-center" />
                      )}
                    </button>
                  );
                })}
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between pl-1">
                  ...
                  <span className="text-[9px] font-bold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-md">Coming Soon</span>
                </div>
                
                {comingSoonCampuses.map((campus) => (
                  <div
                    key={campus.id}
                    className="w-full flex items-center justify-between p-2.5 rounded-xl bg-purple-50/30 border border-purple-100/50 text-left cursor-not-allowed opacity-80"
                  >
                    <div className="flex items-center gap-2">
                      <div className="p-1.5 bg-white rounded-lg text-purple-400 border border-purple-100">
                        <GraduationCap className="w-3.5 h-3.5" />
                      </div>
                      <div>
                    ...
                    <button 
                      ...
                      className="px-2 py-1 bg-white hover:bg-purple-600 hover:text-white border border-purple-200 text-purple-600 font-extrabold text-[9px] rounded-lg transition-all"
                    >
                      Vote
                    </button>
                  </div>
